#!/bin/bash
dialog --title "Install Latest Available Kernel" \
...
response=$?
case $response in
   0) clear && sudo apt install -t bullseye-backports linux-image-amd64 firmware-linux firmware-linux-nonfree firmware-misc-nonfree && echo Successful! && sleep 5 && exit;;
   1) exit;;
   255) exit;;
esac
exec bash;
